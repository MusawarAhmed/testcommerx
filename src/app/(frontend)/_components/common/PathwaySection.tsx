...
const steps = [
  {
    ...
    title: 'Align',
    description:
      'Define Goals, Audience, And Success Metrics To Establish A Clear Go-To-Market (GTM) And Measurement Plan.',
    align: 'right',
  },
  {
    number: '2',
    title: 'Execute',
    description:
      'Launch Integrated Campaigns, Creative Content, And Digital Experiences With Full Tracking Active From Day One.',
    align: 'left',
  },
  {
    number: '3',
    title: 'Optimize',
    description:
      'Analyze real-time data to refine strategies, improve engagement, and maximize return on ad spend (ROAS) and ROI.', // Corrected logical text for Optimize since image had duplicate
    align: 'right',
  },
]
      ...
      <div className="site-containers grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-center">
        ...
        <div className="space-y-8 z-10">
          ...
          <p className="text-[16px] text-gray-700 font-sans leading-relaxed max-w-md">
            Technical And Marketing Complexity Becomes Manageable Through An Integrated, Accountable
            Process Built For Measurable Outcomes.
          </p>
          ...
          <div className="space-y-4 pt-2">
            {features.map((feature, i) => (
              <div key={`feature-${i}`} className="flex items-center gap-3">
                ...
                <span className="text-[16px] font-bold font-sans text-gray-800">{feature}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="relative z-10 p-4">
          <div className="space-y-12 lg:space-y-16">
            {steps.map((step, index) => (
              <div
                key={`step-${index}`}
                className={`relative flex items-start ${step.align === 'right' ? 'lg:flex-row-reverse' : 'lg:flex-row'} gap-8 lg:gap-0`}
              >
                ...
                {/* Content Card */}
                <div
                  className={`w-full lg:w-1/2 pl-20 lg:pl-0 ${step.align === 'right' ? 'lg:pl-16' : 'lg:pr-16'}`}
                >
