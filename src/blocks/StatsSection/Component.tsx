    ...
    <section className="relative py-35 bg-[#0A0A0A] overflow-hidden">
      ...
      <div className="container relative z-10 text-center flex flex-col gap-10">
        <div className="mx-auto space-y-6 max-w-4xl">
          <h2 className="text-[28px] md:text-[48px] font-bold text-white leading-tight">{title}</h2>
          <p className="text-[16px] font-normal text-white/70 mx-auto max-w-2xl">{description}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          {stats.map((stat, index) => (
            ...
            >
              ...
              <p className="text-[14px] md:text-[16px] text-white leading-relaxed">{stat.label}</p>
            </div>
          ))}
        </div>
      </div>
